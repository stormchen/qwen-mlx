# MLX-VLM 本地 AI API 伺服器 🚀

在 Mac（Apple Silicon）上透過 **OpenAI 相容 API** 與 **Anthropic 相容 API** 使用視覺語言模型（VLM），
基於 [mlx-vlm](https://github.com/Blaizzy/mlx-vlm) 封裝。

支援 OpenAI SDK、Anthropic SDK、Claude Desktop、Chatbox 等任何相容客戶端直接呼叫本地模型，包括文字、圖像等多模態輸入。
...
- 🌊 **串流回應** — SSE 串流輸出
- 🔒 **HTTPS 支援** — 可選配 SSL 憑證啟用加密連線
- 🔄 **開機自動啟動** — 透過 macOS launchd 服務管理
...
### OpenAI 相容格式（Chatbox、OpenAI SDK 等）
...
### Anthropic 相容格式（Claude Desktop 等）
...
# Anthropic 格式（Claude Desktop 相容）
...
| 工具 | Base URL | API Key | 格式 |
|------|----------|---------|------|
| **Claude Desktop** | `http://127.0.0.1:8080` | 任意值 | Anthropic |
| **Chatbox** | `http://localhost:8080/v1` | 任意值 | OpenAI |
| **OpenAI SDK** | `http://localhost:8080/v1` | 任意值 | OpenAI |
| **其他工具** | `http://localhost:8080/v1` | 任意值 | OpenAI |

> ⚠️ **模型名稱**必須填寫完整的 HuggingFace ID，例如：`mlx-community/gemma-4-e4b-it-4bit`
...
設定 macOS launchd 服務，讓伺服器開機後自動在背景執行：
...
# 啟用開機啟動
launchctl load ~/Library/LaunchAgents/com.storm.qwen_mlx.plist

# 停用開機啟動
launchctl unload ~/Library/LaunchAgents/com.storm.qwen_mlx.plist

# 查看執行日誌
tail -f server.log
```

> 💡 若專案目錄下存在 `cert.pem` 與 `key.pem`，腳本會自動啟用 HTTPS 模式。
...
```
qwen-mlx/
├── server.py            # 主要服務啟動器（含 Anthropic 相容層）
├── config.py            # 設定管理模組
├── test_api.py          # API 測試腳本
├── setup_autostart.py   # macOS 開機自動啟動設定腳本
